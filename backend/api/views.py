...
class DropdownFiltersView(APIView):
    ...
    def get(self, request):
        filters_data = {
            ...
            "branch": User.objects.exclude(branch__isnull=True).exclude(branch="").values_list("branch", flat=True).distinct().order_by("branch"),
...
class SendEmailAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"success": True, "message": f"Email sent to {sent_count} recipients in batches."}, status=200)
        except Exception as e:
