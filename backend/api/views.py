...
class DropdownFiltersView(APIView):
    ...
    def get(self, request):
        filters_data = {
            ...
            "course": User.objects.exclude(course__isnull=True).exclude(course="").exclude(role__in=["Staff", "Admin"]).values_list("course", flat=True).distinct().order_by("course"),
            "branch": User.objects.exclude(branch__isnull=True).exclude(branch="").exclude(role__in=["Staff", "Admin"]).values_list("branch", flat=True).distinct().order_by("branch"),
